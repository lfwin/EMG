...
class TFRNN:
    def __init__(
        ...
        #pdb.set_trace()
        self.w_conv1 = tf.get_variable("w_conv1"+self.name, shape=(50, num_in, 32), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
        a1 = tf.nn.conv1d(self.input_x, self.w_conv1, stride=1, padding="SAME")
        a1 = tf.nn.relu(a1)
        self.w_conv2 = tf.get_variable("w_conv2"+self.name, shape=(50, 32, 64), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
        a2 = tf.nn.conv1d(a1, self.w_conv2, stride=2, padding="SAME")
        a2 = tf.nn.relu(a2)
        self.w_conv3 = tf.get_variable("w_conv3"+self.name, shape=(50, 64, 64), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
        a3 = tf.nn.conv1d(a2, self.w_conv3, stride=5, padding="SAME")
        a3 = tf.nn.relu(a3)
        self.w_conv4 = tf.get_variable("w_conv4"+self.name, shape=(50, 64, 128), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
        ...
        #modified by xiuyi
        outputs_h, final_state = tf.nn.dynamic_rnn(self.cell, a4, initial_state=self.dyn_rnn_init_states)   
