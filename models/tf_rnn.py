...
class TFRNN:
    def __init__(
        ...
        #pdb.set_trace()
        self.w_conv1 = tf.get_variable("w_conv1"+self.name, shape=(21, num_in, 64), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
        a1 = tf.nn.conv1d(self.input_x, self.w_conv1, stride=1, padding="SAME")
        a1 = tf.nn.relu(a1)
        self.w_conv2 = tf.get_variable("w_conv2"+self.name, shape=(51, 64, 64), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
        a2 = tf.nn.conv1d(a1, self.w_conv2, stride=1, padding="SAME")
        a2 = tf.nn.relu(a2)
        #a2 = tf.nn.dropout(a2, self.keep_prob)
        self.w_conv3 = tf.get_variable("w_conv3"+self.name, shape=(51, 64, 64), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
        ...
        self.w_conv5 = tf.get_variable("w_conv5"+self.name, shape=(31, 128, 128), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
