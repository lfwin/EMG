...
def bi_relu(x):
    ...
    #res = tf.concat([tf.nn.relu(x),  tf.negative(tf.nn.relu(tf.negative(x)))], len(input_shape)-1)
    pos = tf.nn.relu(x)
    neg = tf.negative(tf.nn.relu(tf.negative(x)))

    #pdb.set_trace()
    axis = range(len(input_shape)-1)
    _, var_x = tf.nn.moments(x, axes=axis) 
    m_pos, var_pos = tf.nn.moments(pos, axes=axis) 
    m_neg, var_neg = tf.nn.moments(neg, axes=axis) 

    norm = tf.sqrt(tf.add(2*tf.multiply(m_pos, m_neg), var_x))
    pos = tf.divide(pos, norm+1e-7)
    neg = tf.divide(neg, norm+1e-7)
...
class TFRNN:
    def __init__(
        ...
        self.w_conv3 = tf.get_variable("w_conv3"+self.name, shape=(21, 64, 32), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
        ...
        mask = tf.cast(tf.greater_equal(abs_diff, 2*mean), tf.float32)
    ...
    def evaluate(self, sess, X, Y, training=False):
        if X.shape[0] != 1 and training == False:
            #pdb.set_trace()
            X = np.reshape(X,[1, -1, 1])
            Y = np.reshape(Y,[1, -1, 1])
        # fill (X,Y) placeholders
