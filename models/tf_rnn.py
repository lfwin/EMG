...
class TFRNN:
    def __init__(
        ...
        #pdb.set_trace()
        self.w_conv1 = tf.get_variable("w_conv1"+self.name, shape=(50, num_in, 8), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
        a1 = tf.nn.conv1d(self.input_x, self.w_conv1, stride=1, padding="SAME")
        a1 = tf.nn.relu(a1)
        self.w_conv2 = tf.get_variable("w_conv2"+self.name, shape=(50, 8, 8), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
        a2 = tf.nn.conv1d(a1, self.w_conv2, stride=1, padding="SAME")
        a2 = tf.nn.relu(a2)
        self.w_conv3 = tf.get_variable("w_conv3"+self.name, shape=(3, 8, 8), 
                                            initializer=tf.contrib.layers.xavier_initializer()) # fixm
        a3 = tf.nn.conv1d(a2, self.w_conv3, stride=1, padding="SAME")
        a3 = tf.nn.relu(a3)
        # rnn initial state(s)
        self.init_states = []
        ...
        #modified by xiuyi
        outputs_h, final_state = tf.nn.dynamic_rnn(self.cell, a2, initial_state=self.dyn_rnn_init_states)   
        ...
        if loss_function == tf.squared_difference:
            self.total_loss = tf.add(tf.reduce_mean(loss_function(outputs_o[:, ::50, :], self.input_y)), unreg_loss(outputs_o))
    ...
    def plot_res(self, sess, X, Y):
        ...
        net_o = np.reshape(outputs_o, [-1, 2])
        y = np.reshape(Y, [-1, 2])
        plt.subplot(211)
        plt.plot(net_o[::50, 0], 'r-')
        plt.plot(y[:, 0], 'b-')
        plt.subplot(212)
        plt.plot(net_o[::50, 1], 'r-')
        plt.plot(y[:, 1], 'b-')
        plt.show()
